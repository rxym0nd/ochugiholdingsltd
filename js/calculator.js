...
function calculateLotSize() {
  ...
  var mavenFloor  = OCHUGI_DATA.raymond.mavenStartBalance * 0.95;
...
function renderCalculator() {
  ...
  for (var k = 0; k < keys.length; k++) {
    ...
    instOptions += '<option value="' + key + '"' + (calcState.instrument === key ? ' selected' : '') + '>' + opt.label + '</option>';
  }
  ...
  for (var p = 0; p < RISK_PROFILES.length; p++) {
    ...
    var profileIdx = p;
    profileBtns += '<div onclick="selectProfile(' + profileIdx + ')" style="padding:10px;border-radius:8px;border:1px solid ' + (isActive ? prof.color : 'var(--border)') + ';background:' + (isActive ? 'rgba(255,255,255,0.05)' : 'var(--bg-deep)') + ';cursor:pointer;text-align:center;">';
    profileBtns += '<div style="font-family:var(--font-mono);font-size:0.6rem;color:var(--text-muted);margin-bottom:4px;">' + prof.label + '</div>';
    profileBtns += '<div style="font-family:var(--font-display);font-size:1.1rem;font-weight:700;color:' + prof.color + ';">' + prof.pct + '%</div>';
    profileBtns += '<div style="font-family:var(--font-mono);font-size:0.6rem;color:' + prof.color + ';margin-top:2px;">$' + ((bal * prof.pct) / 100).toFixed(2) + '</div>';
    profileBtns += '</div>';
  }
  ...
  for (var s = 0; s < slPresets.length; s++) {
    ...
    slBtns += '<button class="btn btn-sm ' + (calcState.slPips === preset ? 'btn-gold' : 'btn-outline') + '" onclick="setSlPips(' + preset + ')">' + preset + '</button>';
  }
  ...
  for (var b = 0; b < balPresets.length; b++) {
    ...
    balBtns += '<button class="btn btn-sm ' + (calcState.balance === bPreset ? 'btn-gold' : 'btn-outline') + '" onclick="setBalance(' + bPreset + ')">$' + bPreset.toLocaleString() + '</button>';
  }

  // Build result panel
  var resultHtml = '';
  if (!result) {
    resultHtml = '<div class="card"><div class="card-body" style="text-align:center;padding:40px;color:var(--text-muted);">Enter valid values to calculate.</div></div>';
  } else if (inst.killList) {
    resultHtml = '<div class="card" style="border-color:var(--red);">';
    resultHtml += '<div class="card-body" style="text-align:center;padding:48px 20px;">';
    resultHtml += '<div style="font-size:3rem;margin-bottom:12px;">&#10060;</div>';
    resultHtml += '<div style="font-family:var(--font-display);font-size:1.2rem;font-weight:700;color:var(--red);margin-bottom:8px;">Kill List Instrument</div>';
    resultHtml += '<div style="font-size:0.82rem;color:var(--text-secondary);max-width:280px;margin:0 auto;">This instrument has negative expectancy from your live trade history. Do not calculate a size — remove it from your watchlist entirely.</div>';
    resultHtml += '</div></div>';
  } else {
    // Big lot size display
    resultHtml += '<div class="card" style="border-color:var(--gold-dim);margin-bottom:16px;">';
    resultHtml += '<div class="card-body" style="text-align:center;padding:28px 20px;">';
    resultHtml += '<div class="section-title" style="text-align:center;">Lot Size</div>';
    resultHtml += '<div style="font-family:var(--font-display);font-size:4.5rem;font-weight:800;color:var(--gold);line-height:1;">' + result.lotSize.toFixed(2) + '</div>';
    resultHtml += '<div style="font-family:var(--font-mono);font-size:0.7rem;color:var(--text-muted);margin-top:4px;">standard lots</div>';
    resultHtml += '<div style="font-family:var(--font-mono);font-size:0.6rem;color:var(--text-muted);margin-top:2px;">exact: ' + result.lotExact.toFixed(4) + ' lots</div>';
    resultHtml += '</div></div>';

    // Stats grid
    resultHtml += '<div class="grid-2" style="margin-bottom:16px;">';
    resultHtml += '<div class="stat-card red"><div class="stat-label">Risk Amount</div><div class="stat-value negative" style="font-size:1.3rem;">-$' + result.riskAmount.toFixed(2) + '</div><div class="stat-sub">' + profile.pct + '% of balance</div></div>';
    resultHtml += '<div class="stat-card green"><div class="stat-label">Potential Profit</div><div class="stat-value positive" style="font-size:1.3rem;">+$' + result.potProfit.toFixed(2) + '</div><div class="stat-sub">TP at ' + result.tpPips + ' ' + result.pipLabel + '</div></div>';
    resultHtml += '<div class="stat-card blue"><div class="stat-label">Daily Limit</div><div class="stat-value neutral" style="font-size:1.3rem;">$' + result.dailyLimit.toFixed(2) + '</div><div class="stat-sub">' + result.maxTrades + ' max full-risk trades today</div></div>';
    resultHtml += '<div class="stat-card gold"><div class="stat-label">Maven Buffer</div><div class="stat-value ' + (result.bufferLeft < 100 ? 'negative' : 'gold') + '" style="font-size:1.3rem;">$' + result.bufferLeft.toFixed(2) + '</div><div class="stat-sub">Floor at $' + result.mavenFloor.toFixed(2) + '</div></div>';
    resultHtml += '</div>';
  ...
  for (var li = 0; li < limitsData.length; li++) {
    ...
    limitsHtml += '<div class="list-item" style="padding:10px 20px;">';
    limitsHtml += '<span class="list-item-label">' + lim.label + '</span>';
    limitsHtml += '<span class="list-item-value" style="color:' + lim.color + ';">' + lim.val + '</span>';
    limitsHtml += '</div>';
  }

  container.innerHTML = '<div class="calc-grid">'
    ...
    +       '<div style="display:flex;gap:6px;margin-top:6px;flex-wrap:wrap;">' + balBtns + '</div>'
    +     '</div>'
    ...
    +       '<div style="font-family:var(--font-mono);font-size:0.6rem;color:var(--text-muted);margin-top:4px;">' + inst.note + '</div>'
    +       (inst.killList ? '<div class="alert alert-danger" style="margin-top:8px;padding:8px 12px;font-size:0.78rem;">&#10060; Kill list — negative expectancy. Do not trade.</div>' : '')
    +     '</div>'
    ...
    +       '<div style="display:flex;gap:6px;margin-top:6px;flex-wrap:wrap;">' + slBtns + '</div>'
    +     '</div>'

    // Risk Profile
    +     '<div class="form-group">'
    +       '<label class="form-label">Risk Profile</label>'
    +       '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:4px;">' + profileBtns + '</div>'
    +       '<div style="font-family:var(--font-mono);font-size:0.65rem;color:var(--text-muted);margin-top:6px;">' + RISK_PROFILES[calcState.riskProfile].note + '</div>'
    +     '</div>'
    ...
    +         '<span id="rrDisplay" style="font-family:var(--font-display);font-size:1.3rem;font-weight:700;color:var(--green);min-width:44px;">1:' + calcState.rrRatio + '</span>'
    +       '</div>'
